...
struct ONNXDequantizeLinearOpLowering
  ...
  LogicalResult matchAndRewrite(mlir::ONNXDequantizeLinearOp op, OpAdaptor adaptor,
    ...
    auto toOutputType = [&](Value v) -> Value {
      if (!v)
        return Value();
      if (v.getType() == origOutType)
        return v;
      if (isTensorOfPosit(v.getType()) && isTensorOfF32(origOutType))
        return rewriter.create<mlir::posit::ToF32Op>(loc, origOutType, v).getResult();
      if (isTensorOfF32(v.getType()) && isTensorOfPosit(origOutType))
        return rewriter.create<mlir::posit::FromF32Op>(loc, origOutType, v).getResult();
      if (v.getType() != origOutType)
        return rewriter
            .create<UnrealizedConversionCastOp>(loc, origOutType, v)
            .getResult(0);
      return v;
    };

    // Keep DequantizeLinear on top of ONNX QuantizeLinear / QLinearMatMul in
    // ONNX dialect to preserve exact QDQ integer semantics in ONNX->Krnl.
    if (op.getX().getDefiningOp<mlir::ONNXQuantizeLinearOp>() ||
        op.getX().getDefiningOp<mlir::ONNXQLinearMatMulOp>())
      return rewriter.notifyMatchFailure(
          op, "keep activation/QLinearMatMul dequantize in ONNX for exact QDQ semantics");
    ...
    if (getDenseIntegerTensorFromValue(op.getX(), xRtt, xVals)) {
      ...
      Value out = toOutputType(rankedPosit);
      if (!out)
        return rewriter.notifyMatchFailure(
            op, "failed to convert folded dequantize result to dequantize output type");
      rewriter.replaceOp(op, out);
      return success();
    }
    ...
    if (getDenseFloatTensorFromValue(op.getXScale(), scaleRtt, scaleVals)) {
      ...
      Value out = toOutputType(newOp->getResult(0));
      if (!out)
        return rewriter.notifyMatchFailure(
            op, "failed to convert runtime dequantize result to dequantize output type");
      rewriter.replaceOp(op, out);
      return success();
    }
    ...
    if (remappedX.getType() == origOutType) {
    ...
    if (isTensorOfF32(remappedX.getType())) {
      ...
      Value remappedPos =
          rewriter.create<mlir::posit::FromF32Op>(loc, remappedPosTy, remappedX)
              .getResult();
      Value out = toOutputType(remappedPos);
      if (!out)
        return rewriter.notifyMatchFailure(
            op, "failed to convert fallback dequantize source to output type");
      rewriter.replaceOp(op, out);
      return success();
    }
    if (isTensorOfPosit(remappedX.getType())) {
      Value out = toOutputType(remappedX);
      if (!out)
        return rewriter.notifyMatchFailure(
            op, "failed to convert posit fallback dequantize source to output type");
      rewriter.replaceOp(op, out);
      return success();
    }
