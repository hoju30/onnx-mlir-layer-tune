...
struct ConvertONNXToPositPass
  ...
  void runOnOperation() override {
    ...
    // Hybrid mode: keep all ONNX constants in ONNX dialect and let the later
    // ONNX->Krnl pass lower them.
    target.addLegalOp<ONNXConstantOp>();

    // Keep ONNX DequantizeLinear legal when it consumes a quantized activation
    // or QLinearMatMul result, so we preserve original QDQ integer semantics
    // (quantize/dequantize scale-zp-round-clamp behavior).
    target.addDynamicallyLegalOp<ONNXDequantizeLinearOp>(
        [](ONNXDequantizeLinearOp op) {
          Operation *def = op.getX().getDefiningOp();
          return llvm::isa_and_nonnull<ONNXQuantizeLinearOp, ONNXQLinearMatMulOp>(
              def);
        });
