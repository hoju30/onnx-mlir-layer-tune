...
usage() {
  cat <<'USAGE_EOF'
Usage:
  time_mobilenet_extra_dataset_parallel.sh [txt_dir] [options]

Purpose:
  Run mobilenet extra posit formats in parallel:
...
Options:
  ...
  --master-log FILE     master log file (default: <log-dir>/mobilenetv2-12-extra-parallel.log)
  --console on|off      print progress to stdout (default: off)

Defaults:
  txt_dir   = ./temp/imagenette_val_224
  out_dir   = ./temp/mobilenet_extra
  shape     = 1x3x224x224
  jobs      = 1
...
out_dir="${src_root}/temp/mobilenet_extra"
shape="1x3x224x224"
...
log_msg ""
log_msg "Summary (mobilenet extra formats)"
...
log_msg ""
log_msg "Summary (mobilenet extra formats, GT)"
