...
usage() {
  cat <<'USAGE_EOF'
Usage:
  time_mobilenet11_dataset_parallel.sh [txt_dir] [options for time_model11_dataset_parallel.sh]
...
Examples:
  time_mobilenet11_dataset_parallel.sh --jobs 4 --limit 20
  time_mobilenet11_dataset_parallel.sh ./temp/imagenette_val_224 --jobs 6 --timeout-sec 300
USAGE_EOF
}
