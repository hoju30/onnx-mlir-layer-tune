...
// Container 型別遞迴改元素
static Type convertElementTypeToPosit(Type ety, unsigned nbits, unsigned es,
                                      MLIRContext *ctx) {
  // [FIX] 只轉 **Float**。Integer (e.g. Reshape 的 i64 shape) 必須保留，不然會把 shape 也轉成 posit。
  if (!llvm::isa<FloatType>(ety))
    return ety;
...
static Type convertContainer(Type t, unsigned nbits, unsigned es,
                             MLIRContext *ctx) {
...
PositTypeConverter::PositTypeConverter(unsigned nbits, unsigned es,
  ...
  // 關鍵
  addConversion(
      [nbits, es, ctx](Type t) { return convertContainer(t, nbits, es, ctx); });
