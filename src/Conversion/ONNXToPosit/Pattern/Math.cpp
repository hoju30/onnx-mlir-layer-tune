...
struct ONNXDequantizeLinearOpLowering
  ...
  LogicalResult matchAndRewrite(mlir::ONNXDequantizeLinearOp op, OpAdaptor adaptor,
    ...
    if (auto qop = xBase.getDefiningOp<mlir::ONNXQuantizeLinearOp>()) {
      ...
      // Use strict QDQ semantics by default so scale/zero_point/axis handling
      // matches ONNX behavior and runtime dequantize axis broadcasting.
      constexpr bool preferFractionalQDomain = false;
